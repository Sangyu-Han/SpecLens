...
def main():
    ...
    for k in range(60):
        ...
        pool.append(("mathctx", sents, f"Question: how many {it} does {nm} have now? Answer: {nm} now has", " " + str(ansn)))
    for k in range(70):
        ...
        if tgt is None:
            continue
        si, m = tgt
        pool.append(("needle", sents, "According to the text, " + sents[si][:m.start()].rstrip(), " " + m.group(1)))
    for k in range(50):
        ...
        pool.append(("mhop", sents, f"Question: the job of {a}'s colleague? Answer: The colleague of {a} works as a", " " + jb))

    picked = []
    counts = {}
    for fam, sents, query, ans_str in pool[:0]:
        ...
        with torch.no_grad():
            lg1 = model(input_ids=torch.tensor([idl], device=dev)).logits[0, -1]
        if int(lg1.argmax()) != a_ids[0]:
            continue
        counts[fam] = counts.get(fam, 0) + 1
        if counts[fam] == 1:
            picked.append((fam + "01", sents, query, ans_str))
        break
    picked.append(("everestMCQ", split_sents(EVEREST),
                   "Question: Who made the first documented ascent of Everest? Options: A) George Mallory and Andrew Irvine. B) Tenzing Norgay and Edmund Hillary. C) Wang Fuzhou, Gonpo, and Qu Yinhua. D) The 1921 British reconnaissance team. Answer: The correct option is",
                   " B"))
    ...
    for cname, sents, query, ans_str in picked:
        ...
        for mname, arr in maps.items():
            ...
            pdf_heatmap(words_q, np.clip(aa / (np.abs(aa).max() + 1e-9), -1, 1),
                        path=f"{OUT}/cmp34_{cname}_v8q.pdf", backend="xelatex", delete_aux_files=True)
