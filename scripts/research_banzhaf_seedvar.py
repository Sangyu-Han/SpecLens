#!/usr/bin/env python3
"""HONESTY CHECK: is the restricted-range Banzhaf > AttnLRP win robust, or within run-to-run noise?
The cost M-sweep suggested the per-case insertion AUC fluctuates ~+-0.1-0.15 across seeds. Here we
run the Banzhaf (lo=0.5, fixed M) with K independent SEEDS per case, get per-case mean+-std, and do
a PAIRED comparison vs AttnLRP (deterministic). Verdict = is mean(Banzhaf)-AttnLRP > 0 beyond the
combined seed+case variance?"""
...
def main():
    ...
    for j, (label, text) in enumerate(raw_all):
        ...
        if len(cases) >= 1 + args.n_imdb and all(v >= 1 for v in binc.values()):
            break
    print(f"cases: {[c[0] for c in cases]} | K={args.K} seeds, M={args.M}", flush=True)
    ...
    def run(key, prompt, ans_id):
        ...
        def banzhaf(seed):
            ...
            while j < args.M:
                ...
                for _ in range(nb):
                    pf = LO + (1 - LO) * float(torch.rand(1, generator=g, device=dev).item())
                    zb.append(fix((torch.rand(T, generator=g, device=dev) < pf).float()))
                Z = torch.stack(zb); Rs.append(sig(Z)); Zs.append(Z); j += nb
            Z = torch.cat(Zs); R = torch.cat(Rs); n1 = Z.sum(0)
            s1 = (Z * R[:, None]).sum(0) / n1.clamp(min=1); s0 = ((1 - Z) * R[:, None]).sum(0) / (Z.shape[0] - n1).clamp(min=1)
            return ins_auc((s1 - s0).cpu().numpy())

        aucs = [banzhaf(s) for s in range(args.K)]
        a = attn_cache.get(key)
        ia = ins_auc(np.asarray(a, np.float32)) if a is not None and len(a) == T else float("nan")
        return dict(T=T, bz_mean=float(np.mean(aucs)), bz_std=float(np.std(aucs)), bz_aucs=aucs, ins_attnlrp=ia)

    rows = []
    for key, prompt, ans_id in cases:
        r = run(key, prompt, ans_id); r["key"] = key; rows.append(r)
        print(f"  {key:8s} T={r['T']:4d} | Banzhaf {r['bz_mean']:.3f}+-{r['bz_std']:.3f} (seeds {[round(x,2) for x in r['bz_aucs']]}) "
              f"| attnlrp={r['ins_attnlrp']:.3f} | diff={r['bz_mean']-r['ins_attnlrp']:+.3f}", flush=True)

    diffs = [r["bz_mean"] - r["ins_attnlrp"] for r in rows if r["ins_attnlrp"] == r["ins_attnlrp"]]
    bzm = np.mean([r["bz_mean"] for r in rows]); atm = np.nanmean([r["ins_attnlrp"] for r in rows])
    within_std = np.mean([r["bz_std"] for r in rows])
    print("\n=== VERDICT: restricted Banzhaf vs AttnLRP (per-case mean over K seeds) ===")
    print(f"  mean Banzhaf={bzm:.3f}  AttnLRP={atm:.3f}  diff={bzm-atm:+.3f}")
    print(f"  per-case diff: mean={np.mean(diffs):+.3f} stderr={np.std(diffs)/np.sqrt(len(diffs)):.3f} "
          f"| Banzhaf>AttnLRP on {int(np.sum(np.array(diffs)>0))}/{len(diffs)} cases")
    print(f"  within-case seed std (avg)={within_std:.3f}  (the run-to-run noise on a single AUC)")
    print(f"  => win is {'ROBUST' if np.mean(diffs)-np.std(diffs)/np.sqrt(len(diffs)) > 0 else 'WITHIN NOISE'} "
          f"(mean diff vs paired stderr)")
    json.dump(rows, open(os.path.join(REPO, "outputs", "banzhaf_seedvar.json"), "w"), indent=2)
