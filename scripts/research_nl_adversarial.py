...
def main():
    ...
    def v81_full(ids, ctx, ranges, qtoks, ans_pos, sents):
        ...
        qvals = np.array([loo.get(t, 0.0) for t in qtoks])
        top5 = np.argsort(-qvals)[:5]
        print("    qLOO top5: " + " ".join(f"'{wt(qtoks[int(i)])}':{qvals[int(i)]:.2f}" for i in top5)
              + " | B-label: " + " ".join(f"{qvals[i]:.2f}" for i, t in enumerate(qtoks) if wt(t) == "B"), flush=True)
        qn = relu_norm(qvals) if len(qvals) else qvals
    ...
    pool = []
    for k in range(30):
        nm = NAMES[int(rng.integers(len(NAMES)))]; it = ITEMS[int(rng.integers(len(ITEMS)))]
        v3 = int(rng.integers(2, 7)); v4 = int(rng.integers(2, 7)); tot = v3 + v4
        nm2 = NAMES[int(rng.integers(len(NAMES)))]
        pa = f"{nm}'s count found {v3} red {it} in the basket."
        pb = f"{nm}'s count also found {v4} green {it}."
        pc = f"{nm2}'s recount confirmed {v3} red {it}."
        pd = f"{nm2}'s recount also confirmed {v4} green {it}."
        nf = int(rng.integers(6, 9)); fl = [fillB[int(i)] for i in rng.choice(len(fillB), nf, replace=False)]
        sents = list(fl)
        pos = sorted(rng.choice(nf + 1, 4, replace=True))
        for j, s in enumerate([pa, pb, pc, pd]):
            sents.insert(min(pos[j] + j, len(sents)), s)
        q = f"Question: how many {it} are in the basket now? Answer: The basket now has"
        pool.append(("orand", sents, q, " " + str(tot), dict(P1=[sents.index(pa), sents.index(pb)], P2=[sents.index(pc), sents.index(pd)])))
    picked = []
    for fam, sents, query, ans_str, meta in pool:
        ...
        ap = [len(idl)]
        ids_f = torch.tensor([idl + a_ids], device=dev)
        def pathmask(si_list):
            m = np.zeros(L0)
            for si in si_list:
                for t in range(*rgs[si]):
                    m[cp0[t]] = 1.0
            return m
        vv = fwd_v(ids_f, ctx0, [pathmask(meta["P1"]), pathmask(meta["P2"]), np.zeros(L0)], ap)
        ok1 = vv[0] - vv[2] > 0.25; ok2 = vv[1] - vv[2] > 0.25
        if not (ok1 and ok2):
            print(f"  [dbg] full-OK but path gains {vv[0] - vv[2]:.2f}/{vv[1] - vv[2]:.2f}", flush=True)
            continue
        print(f"  valid OR-of-AND: P1gain={vv[0] - vv[2]:.2f} P2gain={vv[1] - vv[2]:.2f} P1={meta['P1']} P2={meta['P2']}", flush=True)
        picked.append(("orand01", sents, query, ans_str))
        ORMETA[0] = meta
        break
    ...
    for cname, sents, query, ans_str in picked:
        ...
        for mname, arr in maps.items():
            ...
            pdf_heatmap(words_q, np.clip(aa / (np.abs(aa).max() + 1e-9), -1, 1),
                        path=f"{OUT}/cmp33_{cname}_{mname}.pdf", backend="xelatex", delete_aux_files=True)
