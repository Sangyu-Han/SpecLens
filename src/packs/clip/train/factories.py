...
from src.packs.clip.dataset.builders import build_imagefolder_dataset
from src.packs.clip.models.adapters import create_clip_store
...
def load_model(
    ...
    **kwargs,
    ...
    """Create a CLIP model using either keyword or legacy positional arguments."""

    # Legacy path: (model_cfg, device, logger) → tuck device/ignore logger.
    if device is None:
        if not positional:
            raise TypeError("'device' must be provided either as a positional or keyword argument")
        device, *positional = positional

    # Extract optional logger positional for backward compatibility.
    if positional:
