...
def _load_state_dict(model: nn.Module, ckpt_path: Path, *, strict: bool = False) -> None:
    ...
    if isinstance(state, dict):
        if "state_dict" in state:
            ...
            state = state["model"]
    if isinstance(state, dict):
        # Drop possible DistributedDataParallel prefixes
        state = {k.replace("module.", "", 1): v for k, v in state.items()}
    missing, unexpected = model.load_state_dict(state, strict=strict)
    LOGGER.info(
        "[clip] load_state_dict strict=%s missing=%d unexpected=%d", strict, len(missing), len(unexpected)
    )
...
def load_clip_model(
    ...
    **_,
    ...
    # Avoid loading two checkpoints: disable pretrained flag if a manual checkpoint is supplied.
    ...
    if ckpt_path:
        ...
        LOGGER.info("[clip] Loaded pretrained weights for %s", name)

    # Optional casting (e.g., bf16/fp16).
    dtype = model_cfg.get("dtype")
    if dtype:
        try:
            model = model.to(getattr(torch, dtype))
        except AttributeError as exc:
            raise ValueError(f"Unsupported dtype '{dtype}' for CLIP model conversion") from exc

    return model


__all__ = ["load_clip_model"]
