...
DEFAULT_MODEL = "facebook/dinov2-base-imagenet1k-1-layer"
...
def load_dinov2_model(
    ...
    **_,
    ...
    """Load a DINOv2 model from HuggingFace Transformers."""
    cfg = model_cfg or {}
    ...
    for_image_classification = bool(
        cfg.get("for_image_classification", cfg.get("image_classification", False))
    )

    model_cls = AutoModelForImageClassification if for_image_classification else AutoModel
    ...
    if pretrained and rank == 0:
        LOGGER.info("[dinov2] Loaded pretrained weights for %s", name)

    return model


__all__ = ["load_dinov2_model"]
